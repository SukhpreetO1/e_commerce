...
$add_category_types_input_name = $add_category_types_name_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (empty($add_category_types_input_name)) {
      $add_category_types_name_err = "Category name is required.";
   } elseif (strlen($add_category_types_input_name) < 3 || strlen($add_category_types_input_name) > 15) {
      $add_category_types_name_err = "Category name must be between 3 and 15 characters long.";
      ...
      if (mysqli_stmt_num_rows($check_stmt) > 0) {
         ...
         mysqli_stmt_bind_param($insert_stmt, "s", $add_category_types_input_name);
         mysqli_stmt_execute($insert_stmt);
         $response['success'] = "Category types created successfully.";
