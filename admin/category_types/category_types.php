   ...
   function category_types_plus_icon(url) {
      $.ajax({
         ...
         success: function(data) {
            $(".container").empty();
            $('.container').html(data);
            var new_url = window.location.href.replace('?tab=categories_types', '?tab=add_categories_types');
      ...
      $.ajax({
         ...
         success: function(data) {
            ...
            if (!$(data).find('.homepage_sidebar').length) {
               container.html(data);
               var new_url = window.location.href.replace('?tab=categories_types', '?tab=dashboard');
               history.pushState(null, null, new_url);
            }
         },
         error: function(e) {
